...
function real_ip($type=0){
	...
	// Final check: convert any remaining IPv6 localhost
	if ($ip === '::1') {
		$ip = '127.0.0.1';
	}
	return $ip;
...
function showmsg($content = "未知异常", $type = 4, $back = false)
{
    $panel = "info";
    if($type == 1) $panel = "success";
    elseif($type == 2) $panel = "info";
    elseif($type == 3) $panel = "warning";
    elseif($type == 4) $panel = "danger";
    
    $redirect_js = "history.back(-1)";
    if($back) $redirect_js = $back;
    elseif($type == 1) $redirect_js = "window.location.href=window.location.href";
    ...
    echo "<div class=\"panel panel-".$panel."\">
        ...
        <div class=\"panel-body\" style=\"text-align:center;padding:30px;\">
            <div style=\"font-size:16px;margin-bottom:15px;\">".htmlspecialchars($content)."</div>
            <div style=\"color:#999;font-size:13px;\"><i class=\"fa fa-clock-o\"></i> <span id=\"countdown\">2</span> 秒后自动跳转...</div>
        </div>
    </div>
    <script>
    (function(){
