...
case 'setBlock':
	...
	if($DB->exec($sql)!==false)exit('{"code":0,"msg":"淇敼鎴愬姛锛?}');
	else exit('{"code":-1,"msg":"淇敼澶辫触['.$DB->error().']"}');
break;
case 'delFile':
	$id=intval($_GET['id']);
	$row=$DB->getRow("select * from pre_file where id='$id' limit 1");
	if(!$row)
		exit('{"code":-1,"msg":"褰撳墠鏂囦欢涓嶅瓨鍦紒"}');
	$result = $stor->delete($row['hash']);
	$sql = "DELETE FROM pre_file WHERE id='$id'";
	if($DB->exec($sql))exit('{"code":0,"msg":"鍒犻櫎鏂囦欢鎴愬姛锛?}');
	else exit('{"code":-1,"msg":"鍒犻櫎鏂囦欢澶辫触['.$DB->error().']"}');
break;
case 'operation':
	$status=intval($_POST['status']);
	$checkbox=$_POST['checkbox'];
	if(!$checkbox)exit('{"code":-1,"msg":"鏈€変腑鏂囦欢"}');
	$i=0;
	if($status == 2)$opname = '瑙ｅ皝';
	elseif($status == 1)$opname = '灏佺';
	else $opname = '鍒犻櫎';
	...
	exit('{"code":0,"msg":"鎴愬姛'.$opname.$i.'涓枃浠?}');
break;
case 'getFileInfo':
	$id=intval($_GET['id']);
	$row=$DB->getRow("select * from pre_file where id='$id' limit 1");
...
case 'saveFileInfo':
	...
	if(empty($name))exit('{"code":-1,"msg":"鏂囦欢鍚嶇О涓嶈兘涓虹┖"}');
	if($ispwd==1 && !empty($pwd)){
        if (!preg_match('/^[a-zA-Z0-9]+$/', $pwd)) {
			exit('{"code":-1,"msg":"涓嬭浇瀵嗙爜鍙兘涓哄瓧姣嶅拰鏁板瓧"}');
        }
	}
	...
	if($DB->exec($sql, $data)!==false)exit('{"code":0,"msg":"淇敼鏂囦欢淇℃伅鎴愬姛锛?}');
	else exit('{"code":-1,"msg":"淇敼鏂囦欢淇℃伅澶辫触['.$DB->error().']"}');
break;
default:
